  ...
  def create
    ...
    respond_to do |format|
      if @subscription.save
        ...
        format.html do
          redirect_to category_path(@category), notice: "You have successfully subscribed to #{@category.name}."
        end
        format.turbo_stream do
          render turbo_stream: turbo_stream.replace("category_subscription_#{@category.id}", partial: 'categories/subscription',
                                                                                             locals: { category: @category })
        end
      else
        format.html do
          redirect_to category_path(@category),
                      alert: "Failed to subscribe: #{@subscription.errors.full_messages.join(', ')}"
        end
