  ...
  def toggle_like
    ...
    respond_to do |format|
      format.turbo_stream do
        render turbo_stream: turbo_stream.replace("post#{@post.id}actions", partial: 'posts/post_actions',
                                                                            locals: { post: @post })
      end
    ...
    respond_to do |format|
      ...
      format.html do
        redirect_back(fallback_location: root_path, alert: 'An error occurred while processing your request')
      end
    end
  end
end
