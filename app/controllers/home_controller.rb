...
class HomeController < ApplicationController
  before_action :load_posts, only: %i[index main]
  ...
  def load_posts
    ...
    @posts = if defined?(Kaminari) && posts_query.respond_to?(:page)
               posts_query.page(params[:page])
             else
               posts_query.to_a
             end
  ...
  def gallery
    ...
    @posts = Post.joins(:categories).where(visibility: :visible).includes(:user, :categories).distinct.select do |post|
      post.image.present?
    end
    @selected_category = nil
  end

  def gallery_single; end

  def main; end

  def starter_page; end

  def test_turbo
    respond_to do |format|
      format.turbo_stream do
        render turbo_stream: turbo_stream.replace('turbo-result',
                                                  "<div id='turbo-result'>Turbo is working! Time: #{Time.current}</div>")
      end
